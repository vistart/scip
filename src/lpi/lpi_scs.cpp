...
{  /*lint --e{715}*/
    ...
    for (int i = 0; i < ncols; ++i) {
        ...
        c[i] = obj[i];
        SCIPdebugMessage("c[%d]: %s, obj: %f, [%f, %f], beg[i], ind[i], val[i]: (%d, %d, %f)\n", i, colnames[i], obj[i], lb[i], ub[i], beg[i], ind[i], val[i]);
    }
    lpi->scsdata->c = c;
    lpi->n = n;
    lpi->scsdata->P = ConstructPMatrix(n);
    /**
     * TODO:
     * 1. 接收变量边界。
     * 2. 第1条的单元测试。
...
)
{
    ...
    {
        lpi->constraints[row_index * lpi->n + i] = val[row_index * lpi->n + i];
    }
#ifdef SCIP_DEBUG
    for (int i = 0; i < lpi->n; ++i)
    {
...
{  /*lint --e{715}*/
    ...
    for (int i = 0; i < nrows; ++i)
    {
        ...
        SCIPdebugMessage(" <= %f\n", rhs[i]);
        addConstraintsByRow(lpi, i, lhs, rhs, beg, ind, val);
    }
    // lpi->nrows += nrows;
